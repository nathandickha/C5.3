  ...
  setupStepExtensionSlider() {
    ...
    slider.addEventListener("input", () => {
      ...
      if (this.getStepBenchMode?.() === "stepsOnly") {
        ...
        sortedSteps.forEach((step) => {
          const idx = Number.isFinite(Number(step.userData?.stepIndex))
            ? Number(step.userData.stepIndex)
            : sortedSteps.indexOf(step);
          byIndex.set(idx, step);
        });
        ...
        sortedSteps.forEach((step) => {
          ...
          let targetRun = currentRuns.get(stepIndex) || getCurrentRun(step);

          if (stepIndex === selectedTier) {
            targetRun = targetSelectedRun;
          } else if (stepIndex > selectedTier) {
            // Move the lower/deeper tiers with the selected one. The max guard
            // preserves the tier hierarchy if a previous edit made a lower tier
            // smaller than it should be.
            targetRun = Math.max(
              Math.max(0.05, targetRun + delta),
              priorRun + tierOffset
            );
          } else {
