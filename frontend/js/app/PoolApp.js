...
// --------------------------------------------------------------
  ...
  syncStepExtensionSliderForSelectedStep() {
    ...
    if (this.getStepBenchMode?.() === "stepsOnly" && this.selectedStep?.geometry) {
      const geo = this.selectedStep.geometry;
      if (!geo.boundingBox) geo.computeBoundingBox();
      const bb = geo.boundingBox;
      const baseLen = bb.max.x - bb.min.x;
      const sx = this.selectedStep.scale?.x ?? 1;
      const length = baseLen > 0 ? baseLen * sx : (Number(this.selectedStep.userData?.stepRun) || 0.3);
      const maxLen = Math.max(1.5, Number(this.poolParams?.length) || 1.5);
      slider.min = "0.05";
      slider.max = String(maxLen);
      slider.value = String(length);
      if (output) output.textContent = length.toFixed(2) + " m";
      return;
    }
  ...
  setupStepExtensionSlider() {
    ...
    slider.addEventListener("input", () => {
      ...
      if (selectedStepIndex === 1 && this.getStepBenchMode?.() !== "stepsOnly") {
      ...
      // Steps Only mode is wall-backed/nested. The normal generated layout keeps
      // tiers 300 mm apart, but the Step Extension slider is a per-tier override:
      // it extends only the selected tier, instead of resizing the whole stack.
      if (this.getStepBenchMode?.() === "stepsOnly") {
        const selectedTier = Math.max(0, Number.isFinite(selectedIndex) ? selectedIndex : 0);
        const customRuns = { ...(this.poolParams.stepsOnlyStepRuns || {}) };
        customRuns[String(selectedTier)] = Math.max(0.05, val);
        ...
        sortedSteps.forEach((step) => {
          ...
          // Capture the original generated run once, before any per-tier slider
          // edits. Non-selected tiers keep this run, so the selected tier can
          // break away from the fixed 300 mm spacing without the shape collapsing.
          if (!Number.isFinite(Number(step.userData?.defaultStepsOnlyRun))) {
            step.userData.defaultStepsOnlyRun = baseLen * (step.scale?.x ?? 1);
          }

          const override = this.poolParams.stepsOnlyStepRuns?.[String(stepIndex)];
          const overrideRun = Number(override);
          const targetRun = Number.isFinite(overrideRun) && overrideRun > 0
            ? overrideRun
            : Number(step.userData.defaultStepsOnlyRun || step.userData.stepRun || baseLen);

          step.scale.x = Math.max(0.05, targetRun) / baseLen;
          step.position.x = wallX + Math.max(0.05, targetRun) * 0.5;
          step.userData.stepRun = Math.max(0.05, targetRun);
          this.updateScaledBoxTilingUVs(step);
        });
