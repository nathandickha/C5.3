...
// --------------------------------------------------------------
  ...
  previewStepWidthSlider(widthValue) {
    ...
    steps.forEach((step) => {
      ...
      let targetWidth;
      if (stepBenchMode === "stepsOnly") {
        // Steps Only has its own live width preview. Do not preview stepIndex 1
        // as the full-width bench; grow each tier from the selected/narrow
        // width so the preview matches the tiered-cake step-only layout.
        const tierGrowth = position === "center" ? tierOffset * 2 * idx : tierOffset * idx;
        targetWidth = THREE.MathUtils.clamp(narrowWidth + tierGrowth, 0.05, fullWidth);
      } else {
