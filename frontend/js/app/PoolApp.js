...
// --------------------------------------------------------------
  ...
  setupStepExtensionSlider() {
    ...
    slider.addEventListener("input", () => {
      ...
      // Steps Only mode is a nested/tiered-cake set. The extension slider must
      // resize the whole tier stack together, not stretch one selected slab and
      // then chain the remaining steps after it. Keep every tier backed to the
      // entry wall and preserve the fixed 300 mm run offset between tiers.
      if (this.getStepBenchMode?.() === "stepsOnly") {
        ...
        const baseRun = Math.max(0.05, val - tierOffset * selectedTier);
        ...
        sortedSteps.forEach((step) => {
          ...
          const targetRun = Math.max(0.05, baseRun + tierOffset * Math.max(0, stepIndex));
